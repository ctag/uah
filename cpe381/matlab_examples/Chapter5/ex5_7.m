...
if ind==1.
    ...
    subplot(223)
    ezplot(abs(Y1),[-20,20]); grid; axis([-20 20 -0.1 1.5]);title(' '); xlabel('\Omega'); ylabel('|Y_1(\Omega)|')
    subplot(224)
    ezplot(atan(imag(Y1)/real(Y1)),[-20,20]); grid; title(' '); xlabel('\Omega'); ylabel('<Y_1(\Omega)')
    axis([-20 20 -1.5 1.5]);
    xlabel('\Omega')
else
