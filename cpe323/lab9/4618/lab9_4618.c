...
void Serial_Initialize(void)
{
  ...
  UCA0BR1 = 0; 				// Excellent documentation here in the demo code!
...
void main(void)
{
	...
	for (int z = 5000; z > 0; z--);	// Delay to allow baud rate stabilize
	...
	while (1)
	{
		...
		for(int i = 0; i < 21; i++) {
			ch = serialMessage[i];
			UART0_putchar(ch); // print the greeting message
		}
		
		while(rx_flag != 1); // wait until receive the character
		rx_flag = 0;            // clear rx_flag
		
		UART0_putchar(' '); 
		UART0_putchar(ch);	// Echo answer
